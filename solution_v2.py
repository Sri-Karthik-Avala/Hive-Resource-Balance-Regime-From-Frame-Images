...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        q = torch.softmax(self.cell(f3), 1).mean(dim=(2, 3))
        t = q[:, :3].sum(1, keepdim=True)
        s = (q[:, :3] + 1e-3) / (t + 3e-3)
        b, fo, e = s[:, 0], s[:, 1], s[:, 2]
        room = EMPTY_T - e
        z_b = torch.minimum(b - DOM_T, room)
        z_f = torch.minimum(fo - DOM_T, room)
        z_m = torch.minimum(torch.minimum(DOM_T - b, DOM_T - fo), room)
        z_s = e - EMPTY_T
...
def fuse(za, zc):
    return 0.5 * torch.softmax(za, 1) + 0.5 * torch.softmax(zc, 1)
...
def predict(model, arr, bs=64):
    ...
    with torch.no_grad():
        for i in range(0, len(arr), bs):
            ...
            th[:, 0, 0] = 0.82
            th[:, 1, 1] = 0.82
            gz = F.affine_grid(th, (n, 3, IMG_H, IMG_W), align_corners=False)
            xz = F.grid_sample(x0, gz, mode="bilinear", padding_mode="reflection", align_corners=False)
            p = 0.0
            k = 0
            for base in (x0, xz):
                for fl in (False, True):
                    xi = torch.flip(base, dims=[3]) if fl else base
                    za, zc = model(norm(xi))
                    p = p + fuse(za, zc)
                    k += 1
            outs.append((p / k).float().cpu().numpy())
...
def main():
    ...
    for f in range(N_FOLDS):
        if f >= folds_eff[0]:
            print("stop: fold plan", folds_eff[0], flush=True)
            break
        if elapsed() > TRAIN_BUDGET - 120:
            print("stop: budget", flush=True)
            break
        fold_deadline = START + TRAIN_BUDGET * (f + 1) / folds_eff[0]
        ...
        for mod in (model.fc, model.cell):
            for p in mod.parameters():
                head_ids.add(id(p))
        head_ids.add(id(model.scale))
        gh = [p for p in model.parameters() if p.requires_grad and id(p) in head_ids]
        gb = [p for p in model.parameters() if p.requires_grad and id(p) not in head_ids]
        opt = torch.optim.AdamW(
            [{"params": gb, "lr": LR}, {"params": gh, "lr": LR * 4.0}], lr=LR, weight_decay=WD
        )
        base_lrs = [LR, LR * 4.0]
        ...
        while ep < n_ep:
            ...
            et = time.time() - t0
            if ep == 0 and f == 0:
                fit = int((TRAIN_BUDGET - elapsed()) / max(et * TARGET_EPOCHS, 1e-3)) + 1
                folds_eff[0] = int(min(N_FOLDS, max(MIN_FOLDS, fit)))
                print("epoch_time", round(et, 1), "folds_eff", folds_eff[0], flush=True)
            if ep == 0:
                fold_deadline = START + TRAIN_BUDGET * (f + 1) / folds_eff[0]
